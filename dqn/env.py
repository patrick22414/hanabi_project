...
class HanabiEnvironment(object):
    def __init__(
        self, preset="small", players=2, card_knowledge=True, seed=-1, device="cpu"
    ):
        super().__init__()
    ...
    def _make_all_observations(self) -> torch.Tensor:
        all_obs = torch.zeros((self.players, self.obs_shape), device=self.device)
    ...
    def _make_illegal_mask(self):
        illegal_mask = torch.ones(self.max_moves, dtype=bool, device=self.device)
        illegal_mask[
